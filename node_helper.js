/* node_helper.js — Ship24 edition (v3.0.1)
 * Adds listAllTrackers option: when seedTrackers is empty (or by flag),
 * fetch trackers from Ship24 and retrieve their latest results.
 */
...
module.exports = NodeHelper.create({
  start() {
    ...
    console.log('[MMM-MyPackageTracker] node_helper (Ship24) v3.0.1 started');
  },
  ...
  async _collectParcels() {
    ...
    // 1) Seed mode (idempotent POST /v1/trackers/track)
    if (seeds.length > 0) {
      const parcels = [];
    ...
    // 2) List mode (GET /v1/trackers + GET /v1/trackers/:trackerId/results)
    if (this.config.listAllTrackers) {
      const parcels = [];
      let page = 1, more = true;
      const size = Number(this.config.listPageSize || 50);
      ...
      while (more) {
        ...
        for (const t of trackers) {
          try {
            const rid = t.trackerId || t.id;
            if (!rid) continue;
            const results = await this._getTrackerResultsById(rid);
            const parcel = this._normalizeFromShip24Result(
              { data: { tracker: t, shipment: results?.data?.shipment || results?.shipment } },
              { description: t.shipmentReference }
            );
            if (parcel) parcels.push(parcel);
          } catch (e) {
            console.warn('[MMM-MyPackageTracker] results error', e?.message || e);
          }
  ...
  async _listTrackersPage(page = 1, size = 50) {
    ...
    if (!res.ok) {
      const txt = await res.text();
      throw new Error(`Ship24 list trackers failed: ${res.status} ${res.statusText} ${txt}`);
    }
    return res.json().catch(() => ({}));
  },
  ...
  async _getTrackerResultsById(trackerId) {
    ...
    if (!res.ok) {
      const txt = await res.text();
      throw new Error(`Ship24 tracker results failed: ${res.status} ${res.statusText} ${txt}`);
    }
    return res.json().catch(() => ({}));
  },
